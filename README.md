# S21-RK3588-BOX18

## 基本参数

德晟达视频会议终端S21-RK3588-BOX18搭载瑞芯微3588处理器，适用于视频会议中更多的功能扩展包括 AI 同声翻译， 实时会议记录等。三路 HDMI Out 用于视频画面与办公协同三流输出，两路 HDMI In 用于连接云台摄像机和笔记本输入，HUB 接口用于连接会控屏和外设扩展，简单布线实现简约办公。

产品特点：

* 支持HDMI2.0输入 HDMI2.0+HDMI1.4
* 2个USB3.0接口，2个USB2.0接口 USB Type-c接口选配
* 支持8K视频编解码，双显示器4K视频输出
* 支持M.2、SATA多种存储方式


![](./images/27305452021500.png)



## 目录

* [开发板信息](docs/开发板信息.md)
* [官方固件](docs/官方固件.md)


## debug调试口

![](./images/28491116157500.png)


## maskrom短接端

未知、待测量（或者抄答案）


## 相关照片

![](./images/28369976113000.png)

![](./images/28247582531000.png)

![](./images/28269697351200.png)

![](./images/28294642768400.png)

![](./images/28324069362400.png)








